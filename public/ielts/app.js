(function () {
  ...
  function renderToday() {
    ...
    main.innerHTML =
      ...
      moduleCard('03', '词形变换', '先判断空格需要哪种词性，再写出正确派生或屈折形式。', 'forms') +
      moduleCard(
        '04',
        '句子工坊',
  ...
  function startSkillSession(skill) {
    currentView = skill;
    var queue = buildSkillQueue(skill, 10);
    session = {
      type: 'skill',
      words: queue,
      wordIndex: 0,
  ...
  function renderSession() {
    ...
    main.innerHTML =
      ...
      renderQueuePanel() +
      '<article class="panel info-panel"><h3>本词提醒</h3><p>' +
      esc(word.tip) +
      '</p></article>' +
      '</aside>' +
      '</section>';
  ...
  function renderQueuePanel() {
    ...
    return (
      ...
      '<h3>当前队列</h3>' +
      '<p>' +
      (session.type === 'daily' ? '同一个词依次通过四关。' : '到期和薄弱项目优先。') +
      '</p>' +
  ...
  function renderFormsTask(word) {
    ...
    return (
      ...
      Object.keys(POS_LABELS)
        .map(function (pos) {
          ...
          if (task.selectedPos === pos) {
            className += pos === word.form.need ? ' is-selected' : ' is-wrong';
          }
          return (
            '<button class="' +
            className +
      ...
      (answerVisible ? 'disabled' : '') +
      ' />' +
      '<div class="input-actions">' +
      '<button class="secondary-button" type="button" data-action="reveal-form">给我提示</button>' +
      '<button class="primary-button" type="submit"' +
      (answerVisible ? ' disabled' : '') +
      '>检查词形</button>' +
      '</div>' +
      '</form>' +
      '<div class="feedback' +
      (answerVisible ? ' is-wrong' : '') +
      '" id="formFeedback" aria-live="polite">' +
      (answerVisible
        ? '答案：<strong>' +
          esc(word.form.answer) +
          '</strong>。' +
          esc(word.form.explanation) +
          '<div class="input-actions"><button class="secondary-button" type="button" data-action="hide-form-answer">遮住并重写</button></div>'
        : '') +
      '</div>' +
      '</div>' +
  ...
  function handleMainClick(event) {
    ...
    if (action === 'hide-form-answer') return hideFormAnswer();
  ...
  function handleMainSubmit(event) {
    ...
    if (skill === 'forms') checkFormAnswer(new FormData(form).get('answer'));
  ...
  function checkFormAnswer(rawAnswer) {
    if (!session || currentSkill() !== 'forms') return;
    var word = currentWord();
    var answer = String(rawAnswer || '').trim();
    if (!answer) {
    ...
    var task = session.taskState;
    var correct = normaliseAnswer(answer) === normaliseAnswer(word.form.answer);
    if (correct) {
      var firstTry = !task.hadError && !task.answerVisible && !task.retype;
      recordResult(
        word,
        'forms',
        firstTry,
        firstTry ? '词性与词形首次正确' : '提示或看答案后完成重写',
      );
      setFeedback(
        'formFeedback',
        (firstTry ? '正确。' : '重写正确，本次仍进入错项。') + ' ' + esc(word.form.explanation),
        'is-correct',
      );
      disableForm('formInput');
      scheduleAdvance();
      return;
    }

    task.hadError = true;
    task.formAttempts = (task.formAttempts || 0) + 1;
    showFormHint();
  }
  ...
  function showFormHint() {
    ...
    if (attempt === 1) {
      ...
      setFeedback(
        'formFeedback',
        '最小提示：<strong>' +
          esc(edgeHint(word.form.answer)) +
          '</strong>（共 ' +
          compactLength(word.form.answer) +
          ' 个字母）',
        'is-wrong',
        true,
      );
  ...
  function hideFormAnswer() {
    if (!session) return;
    session.taskState.answerVisible = false;
    session.taskState.retype = true;
    renderSession();
    setFeedback('formFeedback', '答案已遮住。根据句子完整重写一次。', '');
  }
